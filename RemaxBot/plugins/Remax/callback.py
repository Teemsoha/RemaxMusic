...
from RemaxBot import YouTube, app
from RemaxBot.core.call import Remax
from RemaxBot.misc import SUDOERS, db
from RemaxBot.utils.database import (is_active_chat,
...
from RemaxBot.utils.formatters import seconds_to_min
from RemaxBot.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from RemaxBot.utils.stream.autoclear import auto_clean
from RemaxBot.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Remax.pause_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Remax.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Remax.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Remax.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )

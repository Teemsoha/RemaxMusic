...
from RemaxBot.misc import HAPP, SUDOERS, XCB
from RemaxBot.utils.database import (get_active_chats,
                                       remove_active_chat,
...
async def vardel_(client, message, _):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            ...
            await message.reply_text(_["heroku_7"].format(check_var))
            os.system(f"kill -9 {os.getpid()} && python3 -m RemaxBot")
